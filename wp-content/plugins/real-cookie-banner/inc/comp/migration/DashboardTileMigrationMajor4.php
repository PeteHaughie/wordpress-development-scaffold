...
class DashboardTileMigrationMajor4 extends \DevOwl\RealCookieBanner\comp\migration\AbstractDashboardTileMigration
{
    ...
    public function actions()
    {
        ...
        $needsUpdate = $updateNotice->needsUpdate();
        ...
        )]), ['linkText' => \__('Show revised design presets', RCB_TD), 'callback' => \add_query_arg(['autofocus[panel]' => BannerCustomize::PANEL_MAIN, 'customAutofocus[rcb-presets]' => 1, 'return' => \wp_get_raw_referer()], \admin_url('customize.php'))])->addAction('website-operator', \__('Provide details of the website operator for self-hosted services', RCB_TD), \join('<br /><br/ >', [\sprintf(
            // translators:
        ...
        )]), ['linkText' => \__('Obtain consent for data processing in unsecure third countries', RCB_TD), 'linkDisabled' => 'performed', 'callback' => $this->getConfigUrl('/settings/consent'), 'performed' => Consent::getInstance()->isDataProcessingInUnsafeCountries(), 'performedLabel' => \__('Feature is enabled.', RCB_TD), 'info' => !empty($servicesWithoutDataProcessingCountries) ? \sprintf('<p>%s</p><ul>%s</ul>', \__('The following services should be reviewed:', RCB_TD), $servicesWithoutDataProcessingCountries) : null])->addAction('footer-links', \__('Specify additional links in the footer of the cookie banner', RCB_TD), \join('<br /><br/ >', [\__('You should link all legally relevant subpages of your website (e.g. privacy policy, legal notice/imprint, terms and conditions) in the cookie banner so that they are quickly accessible. On these pages, the cookie banner should also not be displayed, so as not to put a barrier in front of the pages.', RCB_TD), \__('So far, only privacy policy and legal notice could be linked. From now on you can link any other pages!', RCB_TD)]), ['linkText' => \__('Add more links to the cookie banner footer', RCB_TD), 'callback' => $this->getConfigUrl('/settings')]);
    }
    /**
